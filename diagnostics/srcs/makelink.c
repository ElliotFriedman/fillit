...
/*   Updated: 2018/04/22 21:23:00 by efriedma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	find_coord(char **dptr, t_tetro *piece)
{
	...
	while (y < 4)
	{
		if (dptr[y][x] == '#')
		{
			piece->y[i] = y;
			piece->x[i] = x;
			printf("coordinate %d, %d discovered\n", y, x);
			i++;
		}
		x++;
		if (x == 4)
...
void	one2(char *str, t_tetro *link)
{
	...
	printf("All coordinates found\n");
	printf("Freed 2d array of a single tetrimino\n");
	freemap(dptr, 4);
}
...
t_tetro	*make_list(char **tetro, int size)
{
	...
	i = 0;
	snew = (t_tetro*)malloc(sizeof(t_tetro));
	buf = snew;
	while (i < size)
	{
		one2(tetro[i], snew);
	...
	freemap(tetro, size);
	printf("linked list created with %d links\n", size);
	return (buf);
}
